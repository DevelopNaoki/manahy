...
var vmListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for i := range vmList {
			...
			r_disable := vmList[i].State == "Running" && (vmListOption.Inactive || vmListOption.Saved || vmListOption.Paused)
			s_disable := vmList[i].State == "Off" && (vmListOption.Active || vmListOption.Saved || vmListOption.Paused)
			p_disable := vmList[i].State == "Paused" && (!vmListOption.Paused && !vmListOption.All)
			if r_disable || s_disable || p_disable {
				continue
			} else {
